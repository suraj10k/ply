...
export interface DivergenceAction {
  ...
  description: string;
  // Metadata for applying the resolution automatic steps
  resolutionPlan?: {
    fileToModify?: string;
    description: string;
  };
...
export interface OnboardingInterviewState {
  ...
  data: {
    tables: Array<{ name: string; primaryKey: string; columns: string[] }>;
  };
  references: {
...
export interface CodebaseFacts {
  ...
  detectedRoutes: Array<{ path: string; method: string }>;
  framework: string;
  configFiles: string[];
}
